...
Student readStudBin(int shift) {
	Student stdREAD;
	HANDLE FileHandle = CreateFile("..\\..\\binFile.csv", GENERIC_READ | GENERIC_WRITE, FILE_SHARE_WRITE | FILE_SHARE_READ,
		NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);

	if (FileHandle == INVALID_HANDLE_VALUE) {
		cout << "File opening error: " << GetLastError() << endl;
		system("pause");
	}
	SetFilePointer(FileHandle, 4 + (shift - 1) * sizeof(stdREAD), NULL, FILE_BEGIN);

	DWORD dwBytesRead;
	if (!ReadFile(FileHandle, &stdREAD, sizeof(stdREAD), &dwBytesRead, NULL)) {
		cout << "ReadFile error: " << GetLastError() << endl;
	}
	//cout << stdREAD.num << endl;
	//cout << stdREAD.name << endl;
	//cout << stdREAD.grade << endl;
...
void writeStudBin(Student wrStud, int shift) {
	Student stdWRITE = wrStud;
	Student stdREAD;
	HANDLE FileHandle = CreateFile("..\\..\\binFile.csv", GENERIC_READ | GENERIC_WRITE, FILE_SHARE_WRITE | FILE_SHARE_READ,
		NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);
	...
	SetFilePointer(FileHandle, 4 + (shift-1) * sizeof(stdWRITE), NULL, FILE_BEGIN);
	DWORD dwBytesRead;
	if (!ReadFile(FileHandle, &stdREAD, sizeof(stdREAD), &dwBytesRead, NULL)) {
		cout << "ReadFile error: " << GetLastError() << endl;
	}

	stdWRITE.num = stdREAD.num;
	SetFilePointer(FileHandle, 4 + (shift - 1) * sizeof(stdWRITE), NULL, FILE_BEGIN);
	DWORD dwBytesWritten;
	if (!WriteFile(FileHandle, &stdWRITE, sizeof(stdWRITE), &dwBytesWritten, NULL)) {
