...
void CMainDlg::OnProcess()
{
	...
	{
		MessageBox(L"Process failed", L"Error", MB_OK | MB_ICONERROR);
	}
	else 
		MessageBox(L"File processed", L"Information", MB_OK | MB_ICONINFORMATION);
}
...
bool CMainDlg::ProcessFiles(CString sFileIn, CString sFileOut)
{
	...
	{
		CString sMsg;
		sMsg.Format(L"Can't open FileIn %s", sFileIn);
	...
	{
		CString sMsg;
		sMsg.Format(L"Can't open FileOut %s", sFileOut);
		MessageBox(sMsg, L"Error", MB_OK | MB_ICONERROR);
		return FALSE;
	}
